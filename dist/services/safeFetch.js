...
exports.safeAgents = exports.safeHttpsAgent = exports.safeHttpAgent = void 0;
...
function isBlockedIPv4(ip) {
    ...
    if (a === 169 && b === 254)
        return true; // 169.254/16  link-local (incl. 169.254.169.254 metadata)
    if (a === 172 && b >= 16 && b <= 31)
...
const guardedLookup = (hostname, _options, callback) => {
    dns_1.default.lookup(hostname, { all: true, verbatim: true }, (err, addresses) => {
        if (err)
        ...
        for (const { address } of addresses) {
            if (isBlockedAddress(address)) {
                return callback(new Error(`Blocked request to private address (${address})`), "", 0);
            }
        }
...
exports.safeHttpAgent = new http_1.default.Agent({ lookup: guardedLookup });
exports.safeHttpsAgent = new https_1.default.Agent({ lookup: guardedLookup });
/** Spread into any axios call that fetches a user-supplied URL. */
exports.safeAgents = {
    httpAgent: exports.safeHttpAgent,
    httpsAgent: exports.safeHttpsAgent,
};
